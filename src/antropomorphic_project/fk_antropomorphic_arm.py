...
def main(args=None):
    # initialize
    print("forward_kinematic_matrices")
    ...
    if len(sys.argv) < 4:
        ...
        t1_value = float(sys.argv[1])
        t2_value = float(sys.argv[2])
        t3_value = float(sys.argv[3])
    ...
    # extract position and orientation of arm
    position = [round(v, 5) for v in AO3_evaluated.col(-1)[:3]]
    orientation = f"{AO3_evaluated.row(0)[:3]} {AO3_evaluated.row(1)[:3]} {AO3_evaluated.row(2)[:3]}"
